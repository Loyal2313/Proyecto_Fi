...
while ($usuario != null){ //Recorro el resultado
    echo "<p style='text-align: left'><form action='' style='border: solid 2px black; padding: 3%' method='POST'><span style='font-weight: bold'>Usuario:</span> "
        .$usuario[1]." <input name='borrar' readonly value='".$usuario[0]."'><span style='font-weight: bold'>Última conexión: </span>"
        .$usuario[3]."<br><br><button type='submit' style='margin-left: 5%; background-color: red'>Borrar Usuario</button></form></p>";
    $usuario = $resultado->fetch_array(MYSQLI_BOTH);
}
...
$usuario = $resultado->fetch_array(MYSQLI_BOTH);
while ($usuario != null){ //Recorro el resultado
    echo "<p class='usuario'><form action='' style='border: solid 2px black; padding: 3%' method='POST'><span style='font-weight: bold'>Usuario:</span> "
        .$usuario[1]." <input name='activar' readonly value='".$usuario[0]."'><span style='font-weight: bold'>Última conexión: </span>"
        .$usuario[3]."<br><br><button type='submit' style='margin-left: 5%; background-color: green'>Activar Usuario</button></form></p>";
    $usuario = $resultado->fetch_array(MYSQLI_BOTH);
}
$resultado->free(); //Libero de la memoria
echo "";
